...
local function find_in_files()
  Snacks.picker.grep({ cwd = workspace_root() })
end
...
map("n", "<M-l>", function()
  cycle_all_files(1)
end, { desc = "Cycle files across all panes" })
map("n", "<C-Tab>", function()
  cycle_all_files(1)
end, { desc = "Next open editor" })
map("n", "<C-S-Tab>", function()
  cycle_all_files(-1)
end, { desc = "Previous open editor" })
map("n", "]b", function()
  cycle_pane_files(1)
...
map("n", "<A-Down>", "<cmd>silent! move .+1<cr>==", { desc = "Move line down" })
map("n", "<A-Up>", "<cmd>silent! move .-2<cr>==", { desc = "Move line up" })
...
map("n", "<C-A-e>", function()
  require("config.workspace").move_current_file(1)
end, { desc = "Move file to right editor pane" })
...
map("n", "<F2>", function()
  lsp_action("textDocument/rename", vim.lsp.buf.rename, "symbol rename")
...
map("n", "<C-S-A-r>", function()
  require("config.arabic").toggle()
end, { desc = "Toggle mixed Arabic/English bidi" })
...
map("n", "<leader>wq", close_current_file, { desc = "Close current file" })
map("n", "<leader>wQ", safe_close_window, { desc = "Close current window" })
map("n", "<leader>bd", close_current_file, { desc = "Close current file" })
map("n", "<leader>bD", close_current_file, { desc = "Close current file" })
